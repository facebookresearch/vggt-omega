...
def bilinear_sampler(input, coords, align_corners=True, padding_mode="border", pixel_convention: str = "colmap"):
    ...
    # If input coords are in pixel-space, convert to the expected grid_sample range
    # This wrapper expects coords in pixel units. Optionally shift for COLMAP center convention
    if pixel_convention.lower() == "colmap":
        # Convert pixel-center coords (i+0.5) to edge-based coords if align_corners=False
        # For align_corners=True, centers are at integers already; the +0.5 centers require shifting to match grid_sample's assumption.
        if align_corners:
            raise NotImplementedError
            # Shift to align with pixel centers used by COLMAP
            coords = coords - 0.5
        else:
            # grid_sample with align_corners=False assumes [0, W] and [0, H] at edges
            # Our coords are centers, so no change needed here; leave as-is
            pass
