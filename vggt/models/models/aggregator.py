...
import math
import logging
from vggt.models.layers import SelfAttentionBlock, RopePositionEmbedding, Mlp, LayerScale, PatchEmbed, RMSNorm, SwiGLUFFN
from vggt.models.utils.utils import named_apply
...
class Aggregator(nn.Module):
    ...
    Args:
        ...
        use_flash_attn_global (bool): If True, use Flash Attention for all global attention
            layers (independent of use_flash_attn which controls both frame and global layers).
    """
    ...
    def __init__(
        ...
        rope_coord_norm_denominator: Optional[int] = None,
        ...
        enable_gated_attention: bool = False,
        ...
        use_flash_attn_global: bool = False,
    ):
        super().__init__()
        ...
        if not disable_rope_global:
            ...
            print(
                "Aggregator: global RoPE request ignored; "
                "_process_global_attention expects rope_sincos=None"
            )
            logging.info(
                "Aggregator: global RoPE request ignored; "
                "_process_global_attention expects rope_sincos=None"
        ...
        if force_rope_normalize_coords_max:
            if rope_normalize_coords != "max":
                print(
                    "Aggregator: force_rope_normalize_coords_max=True, overriding "
                    f"rope_normalize_coords={rope_normalize_coords} -> max"
                )
                logging.info(
                    "Aggregator: force_rope_normalize_coords_max=True, overriding "
                    f"rope_normalize_coords={rope_normalize_coords} -> max"
            ...
            if rope_coord_norm_denominator is not None:
                print(
                    "Aggregator: force_rope_normalize_coords_max=True, ignoring "
                    f"rope_coord_norm_denominator={rope_coord_norm_denominator}"
                )
                logging.info(
                    "Aggregator: force_rope_normalize_coords_max=True, ignoring "
                    f"rope_coord_norm_denominator={rope_coord_norm_denominator}"
                )
                rope_coord_norm_denominator = None
        print(f"Aggregator: RoPE normalize_coords={rope_normalize_coords}")
        logging.info(f"Aggregator: RoPE normalize_coords={rope_normalize_coords}")
        if self.match_patch_embed_rope_behavior:
            print("Aggregator: matching patch_embed RoPE behavior (per-block sampling)")
        ...
        self.rope_embed = (
            RopePositionEmbedding(
                ...
                coord_norm_denominator=rope_coord_norm_denominator,
                dtype=self.rope_dtype,
            )
            if rope_freq is not None and rope_freq > 0
            else None
        )
        if self.rope_embed is not None and rope_coord_norm_denominator is not None:
            print(f"Aggregator: using RoPE coord_norm_denominator={rope_coord_norm_denominator}")
            logging.info(f"Aggregator: using RoPE coord_norm_denominator={rope_coord_norm_denominator}")
        ...
        _global_flash = use_flash_attn_global or use_flash_attn
        ...
        print(f"Aggregator use_checkpoint: {self.use_checkpoint}")
        ...
        if uncheckpointed_ratio > 0.0:
            ...
            print(f"Aggregator: {len(self.uncheckpointed_indices)}/{depth} layers skip checkpointing "
        ...
        if last_n_global_specials > 0:
            ...
            print(f"Aggregator: Set last {last_n_global_specials} global layers to 'specials' attention")
        ...
        print(f"Aggregator init_mode: {self.init_mode}")
    ...
    def forward(
        ...
        # Call patch_embed_model with out_indices if supported
        patch_tokens = patch_embed_model(images, out_indices=patch_embed_out_indices)
        
        patch_embed_intermediate = None
        if isinstance(patch_tokens, dict):
            if patch_embed_out_indices is not None and "intermediates" in patch_tokens:
                patch_embed_intermediate = patch_tokens["intermediates"]
            if self.use_dino_clsreg:
