...
def _make_dinov3_vit(
    *,
    ...
    torchhub_cache: bool = True,
    **kwargs,
):
    from ..models.vision_transformer import DinoVisionTransformer
    
    
    # NOTE pos_embed_rope_rescale_coords is input now
    vit_kwargs = dict(
        img_size=img_size,
    ...
    if torchhub_cache:
        ...
        model_name = os.path.basename(_make_dinov3_vit_model_url(
                patch_size=patch_size,
        ...
        folder_name = "_".join(model_name.split("_")[:2])
        model_path = os.path.join(torch_hub_dir, "dinov3", folder_name, model_name)
        print(f"torchhub_cache: Loading DINO v3 model from {model_path}")
        if os.path.exists(model_path):
            state_dict = torch.load(model_path, map_location="cpu")
            # Filter out local_cls_norm related weights
            state_dict = {k: v for k, v in state_dict.items() if "local_cls_norm" not in k}
            # Verify DINO weights before loading
            print(f"[DINO VERIFY] Before load from cache - patch_embed.proj.weight: mean={model.patch_embed.proj.weight.mean().item():.6f}, std={model.patch_embed.proj.weight.std().item():.6f}")
            model.load_state_dict(state_dict, strict=True)
            # Verify DINO weights loaded correctly
            print(f"[DINO VERIFY] After load from cache - patch_embed.proj.weight: mean={model.patch_embed.proj.weight.mean().item():.6f}, std={model.patch_embed.proj.weight.std().item():.6f}")
        else:
        ...
        state_dict = {k: v for k, v in state_dict.items() if "local_cls_norm" not in k}
        # Verify DINO weights before loading
        print(f"[DINO VERIFY] Before load from URL - patch_embed.proj.weight: mean={model.patch_embed.proj.weight.mean().item():.6f}, std={model.patch_embed.proj.weight.std().item():.6f}")
        model.load_state_dict(state_dict, strict=True)
        # Verify DINO weights loaded correctly
        print(f"[DINO VERIFY] After load from URL - patch_embed.proj.weight: mean={model.patch_embed.proj.weight.mean().item():.6f}, std={model.patch_embed.proj.weight.std().item():.6f}")
    else:
        model.init_weights()
    return model
...
def dinov3_vits16(
    ...
    **kwargs,
    ...
    kwargs["version"] = None
    rope_normalize_coords = kwargs.pop("pos_embed_rope_normalize_coords", "separate")
    return _make_dinov3_vit(
        img_size=224,
        patch_size=16,
        in_chans=3,
        pos_embed_rope_base=100,
        pos_embed_rope_normalize_coords=rope_normalize_coords,
