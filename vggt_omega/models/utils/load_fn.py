...
def load_and_preprocess_images_square(image_path_list, target_size=512):
    ...
    Args:
        ...
        target_size (int, optional): Target size for both width and height. Defaults to 512.
...
def load_and_preprocess_images(image_path_list, mode="crop", target_size=512, patch_size=16):
    ...
    Args:
        ...
        target_size (int, optional): Target image size. Defaults to 512.
        patch_size (int, optional): Patch size used for divisibility. Defaults to 16.
    ...
    Notes:
        ...
        - When mode="pad": The function ensures the largest dimension is target_size while maintaining aspect ratio
          and the smaller dimension is padded to reach a square shape.
        - Dimensions are adjusted to be divisible by patch_size for compatibility with model requirements.
    """
    # Check for empty list
    if len(image_path_list) == 0:
        raise ValueError("At least 1 image is required")
