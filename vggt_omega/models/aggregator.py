...
class Aggregator(nn.Module):
    ...
    def __init__(
        ...
        special_global_block_indices: list[int] = [2, 6, 9, 14, 20],
    ) -> None:
        super().__init__()
        ...
        for idx in special_global_block_indices:
            if idx < 0 or idx >= depth:
                raise ValueError(f"special_global_block_indices contains invalid block index {idx}")
            self.global_attn_modes[idx] = "specials"
    ...
    def forward(
        ...
        for block_idx in range(self.depth):
            ...
            tokens = self._run_global_block(
                ...
                embed_dim,
                block_idx,
                self.global_attn_modes[block_idx],
            )
    ...
    def _run_global_block(
        ...
        if mode == "all":
            tokens = tokens.view(batch_size, num_frames * num_tokens, embed_dim)
            tokens = self.global_blocks[block_idx](tokens, None)
            return tokens.view(batch_size, num_frames, num_tokens, embed_dim)

        if mode != "specials":
            raise ValueError(f"Unknown global attention mode: {mode}")

        patch_start_idx = self.patch_start_idx
        special_tokens = tokens[:, :, :patch_start_idx].reshape(batch_size, num_frames * patch_start_idx, embed_dim)
        patch_tokens = tokens[:, :, patch_start_idx:].reshape(
            batch_size,
            num_frames * (num_tokens - patch_start_idx),
            embed_dim,
        )

        special_tokens = self.global_blocks[block_idx](special_tokens, None)
        tokens = torch.cat([special_tokens, patch_tokens], dim=1)

        special_tokens = tokens[:, : num_frames * patch_start_idx].view(batch_size, num_frames, patch_start_idx, embed_dim)
        patch_tokens = tokens[:, num_frames * patch_start_idx :].view(
            batch_size,
            num_frames,
            num_tokens - patch_start_idx,
            embed_dim,
        )
